...
func (l *AddLogic) Add(req types.ReqUserFav) (*types.RespFavDetail, error) {
	...
	_, err = l.svcCtx.Operation.AddUserFav(l.ctx, &operation.UserFavRequest{
		UserId:  req.UserId,
		StockId: req.StockId,
	})
	if err != nil {
		logx.Errorf("收藏股票失败: ", err.Error())
		detailStatus := types.DetailMeta{
			Msg:    "收藏股票失败",
			Status: http.StatusInternalServerError,
		}
