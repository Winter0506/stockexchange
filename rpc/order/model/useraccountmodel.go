...
// 这是查询所有
func (m *defaultUseraccountModel) FindAll() (*[]Useraccount, error) {
	var resp []Useraccount
	Key := fmt.Sprintf("%s", cacheUseraccountUseridPrefix)
	err := m.QueryRow(&resp, Key, func(conn sqlx.SqlConn, v interface{}) error {
		query := fmt.Sprintf("select %s from %s", useraccountRows, m.table)
